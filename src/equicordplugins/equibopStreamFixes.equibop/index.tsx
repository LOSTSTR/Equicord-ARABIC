...
const settings = definePluginSettings({
    ...
    removeResolutionCap: {
        type: OptionType.BOOLEAN,
        description: "Allow resolutions above 720p at 60 frames per second",
        default: true,
        restartNeeded: true,
    },
    forceEncoderSettings: {
        type: OptionType.BOOLEAN,
        description: "Force the encoder to use the configured resolution and framerate",
        default: true,
        restartNeeded: true,
    },
    preventDownscale: {
        type: OptionType.BOOLEAN,
        description: "Prevent Discord from downscaling the stream resolution",
        default: true,
        restartNeeded: true,
    },
    keyframeInterval: {
        type: OptionType.NUMBER,
        description: "Keyframe interval in milliseconds (0 = encoder default, 5000 = every 5 seconds)",
        default: 5000,
        restartNeeded: true,
    },
    minBitrate: {
    ...
    preventFramerateReduction: {
        type: OptionType.BOOLEAN,
        description: "Prevent Discord from reducing the stream framerate when you stop speaking",
        default: true,
        restartNeeded: true,
    },
    bitsPerPixelPct: {
        type: OptionType.NUMBER,
        description: "Bits per pixel percentage for target bitrate (8 = 0.08 bpp, 12 = 0.12 bpp)",
        default: 8,
        restartNeeded: false,
    },
});
...
export default definePlugin({
    ...
    description: "Attempts to fix stream quality on Equibop by tweaking Discord's encoder and quality restrictions.",
    tags: ["Voice"],
