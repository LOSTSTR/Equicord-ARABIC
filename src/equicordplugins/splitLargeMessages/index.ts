...
const settings = definePluginSettings({
    maxLength: {
        ...
        description: "Maximum message length before splitting. Set to 0 for auto-detection.",
        default: 0,
        max: 4000,
        onChange(newValue) {
            if (newValue === 0)
    ...
    disableFileConversion: {
        ...
        description: "When enabled, disables file conversion for long messages.",
        default: true,
    },
    sendDelay: {
    ...
    hardSplit: {
        type: OptionType.BOOLEAN,
        description: "When enabled, splits at the last character instead of the last space/newline.",
        default: false,
    },
    splitInSlowmode: {
        type: OptionType.BOOLEAN,
        description: "Should messages be split if the channel has slow mode enabled?",
    },
    slowmodeMax: {
        type: OptionType.NUMBER,
        description: "Maximum slowmode duration when splitting in it.",
        default: 5,
        min: 1,
        max: 30,
    }
